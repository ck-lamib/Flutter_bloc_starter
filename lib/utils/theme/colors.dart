import 'package:flutter/material.dart';

/// Colors used in the application
abstract class MyAppColors {
  /// Color: 0xFFFFFFFF
  static const Color white = Color(0xFFFFFFFF);

  /// Color: 0xFF000000
  static const Color black = Color(0xFF000000);

  /// Color: 0xFF0C32A4
  static const Color darkBlue = Color(0xFF0C32A4);

  /// Color: 0xFFFFEE02
  static const Color yellow = Color(0xFFFFEE02);

  /// Color: 0xFFE5AB05
  static const Color orange = Color(0xFFE5AB05);

  /// Color: 0xFFF03939
  static const Color red = Color(0xFFF03939);

  /// Color: 0xFF4B94F6
  static const Color blue = Color(0xFF4B94F6);

  /// Color: 0x00000000
  static const Color transparent = Color(0x00000000);

  /// Color: 0xFFE33B2D
  static const Color loadingDarkRed = Color(0xFFE33B2D);

  /// Color: 0xFFEC5E2B
  static const Color loadingLightRed = Color(0xFFEC5E2B);

  /// Color: 0xFF4087F8
  static const Color loadingDarkBlue = Color(0xFF4087F8);

  /// Color: 0xFF6CCAE4
  static const Color loadingLightBlue = Color(0xFF6CCAE4);

  /// Color: 0xFF274E54
  static const Color crtBackground = Color(0xFF274E54);

  /// Green
  static const Color green = Color(0xFF3fBC5C);

  /// Black 25% opacity
  static const Color black25 = Color(0x40202124);

  /// Black 54% opacity
  static const Color black54 = Color(0x8A000000);

  /// Charcoal
  static const Color charcoal = Color(0xBF202124);

  //for customcolor
  static const Color primarycolor = Color(0xFF016D94);
  static const Color primarycolordark = Color(0xff004c67);
  static const Color textcolor = Color(0xFF3A4853);
  static const Color secondaryTextcolor = Color.fromARGB(255, 146, 145, 145);
  static const Color textcolordark = Color.fromARGB(255, 245, 245, 245);
  static const Color appBarTextColor = Color.fromARGB(255, 245, 245, 245);
  static const Color backgroundColor = Color(0xFFF4F4F4);
  static const Color whiteColor = Color(0xFFFFFFFF);
  static const Color redColor = Color(0xFFE84847);
  static const Color orangeColor = Color(0xFFDA8A01);
  static const Color greenColor = Color(0xFF5FB710);
  static const Color lightGreenColor = Color(0xFF6bce12);
  static const Color skyBlueColor = Color(0xFF14CBCC);
}
